...
def create_qualification_test(type_hit:str):
    if type_hit == a_Role:
        ...
        print('Qualification can be only of two types: either architect or builder. No qualification test has been created')
    ...
    if a_Role in type_hit:
        hit = mturk.create_hit(
                **common_kwargs,
                ...
                Title='Play collaborative Minecraft game as an architect for pre-approved gamer!',
                ...
                QualificationRequirements=[{'QualificationTypeId': '32IU3MUE9M649TMT5TLMP0CD6O86EW', 
                                          'Comparator': 'EqualTo',
                                           'IntegerValues':[88]}]
            ) 
        ...
        return hit['HIT']['HITId']
         ...
         hit = mturk.create_hit(
                **common_kwargs, 
                Reward='1.00',
                AssignmentDurationInSeconds=600,
                Title='Play collaborative Minecraft game as a builder for pre-approved gamer!',
...
def create_turk_job(table_hits, table_games, path_target_ids, max_num_steps_game = 30):
    ...
    with TableClient.from_connection_string(conn_str=connect_str_key, table_name=table_games) as table_client_games:
        ...
        for attempt_id, structure_id in games:
            ...
            game_markComplete_flag[game_id] = 0
           
        with TableClient.from_connection_string(conn_str=connect_str_key, table_name=table_hits) as table_client_hits:
            while games:
                for game in list(games):
                    print('For init game is the latest hit is {}'.format(game))
                if is_it_initial:
                    is_it_initial = False 
                    init_hits(games, table_client_hits)
                else:
                    results = get_complete_assignments()
                    print('Inside create job: retrieved results')
                    if results:
                        # add list of Hits and accept and delete only ones which 
                        print('If results exists')
                        processed_hits = []
                        for (attempt_id, structure_id), game_info in dict(games).items():
                            print("dict(games).items:{} ".format(dict(games).items()) )
                            game_id = create_game_id(attempt_id, structure_id) 
                            ...
                            if 'IsBuiltStructureComplete' in merged_entity:
                                if merged_entity['IsBuiltStructureComplete'].lower() == 'yes' and game_markComplete_flag[game_id] == 0: 
                                # this flag is set so that more than one worker needs to validate if structure is complete
                                    game_markComplete_flag[game_id]= 1
...
def create_new_hit(max_num_steps_game, games, table_client_games, table_client_hits, attempt_id, structure_id, game_info, game_id, merged_entity, current_step_id):
    if "architect" in merged_entity["Role"]:
        if merged_entity["IsBuiltStructureComplete"] == 'yes' or merged_entity["StepId"] > max_num_steps_game:
...
def re_create_hit(table_client_hits, attempt_id, structure_id, game_info, game_id, merged_entity, current_step_id):
    if "architect" in merged_entity["Role"]:
            if merged_entity["Role"] == 'architect-normal':
               ...
               game_info["latest_hit"] = re_create_architect_normal(
                                                                                table_client_hits, 
                                                                                attempt_id, 
                                                                                structure_id, 
                                                                                game_id, 
...
def create_architect_normal(table_client_hits, attempt_id, structure_id, game_id, current_step_id):
    ...
    create_new_row4new_hit(
                            ...
                            new_hit_type,
                            hit_input)
    print("Creating architect-normal new_hit_id {} new_hit_type {} current_step_id {}  Done!".format(new_hit_id, new_hit_type, current_step_id))
    return new_hit_id 

def re_create_architect_normal(table_client_hits, attempt_id, structure_id, game_id, merged_entity, current_step_id):
    print("Creating architect-normal. Done!")
    new_hit_type = "architect-normal"
    hit_input = {}
    template = create_template_given_type(
...
def create_architect_cq(table_client_hits, attempt_id, structure_id, game_id, merged_entity, current_step_id):
...
def re_create_architect_cq(table_client_hits, attempt_id, structure_id, game_id, merged_entity, current_step_id):
    print("Creating architect-cq game id {} current step i {} ".format(game_id, current_step_id))
    new_hit_type = "architect-cq"
    hit_input = {}
...
def create_builder_cq(table_client_hits, attempt_id, structure_id, game_id, merged_entity, current_step_id):
...
def re_create_builder_cq(table_client_hits, attempt_id, structure_id, game_id, merged_entity, current_step_id):
    print('Creating builder-cq')
    new_hit_type = "builder-cq"
    hit_input = {} 
...
def create_builder_normal(table_client_hits, attempt_id, structure_id, game_id, merged_entity, current_step_id):
    template = ''
...
def re_create_builder_normal(table_client_hits, attempt_id, structure_id, game_id, merged_entity, current_step_id):
    template = ''
...
def create_new_row4new_hit(table_client_hits, attempt_id, structure_id, current_step_id, new_hit_id, hit_type, hit_input):
    entity_to_insert = {}
    entity = {
...
def create_screenshot_path_till_view(game_id, step_id):
    if  (step_id % 2) == 1: 
        ...
        print("The path to builder intermediate world is [/builder-data/{}/step-{}]".format(game_id,step_id))    
...
def init_hits(games, table_client_hits):
    for attempt_id, structure_id in games:
        template = create_template_given_type(
            ...
            gameId=create_game_id(attempt_id, structure_id), 
            structureId=structure_id, 
            screenshotPath='default/view_',
            north='north',
            south='south',
...
def initialize_new_games(path_target_ids, table_games, table_client_games):
    ...
    for structure_id in structure_ids:
        ...
        print("PartitionKey eq '{structureId}'".format(structureId=structure_id))
